...
const sendVerificationCode = async () => {
  ...
  const result = await authClient.startPhoneChange({
    phone: form.newPhone
  })
...
const verifyPhone = async () => {
  ...
  let result: unknown
  if (flowId.value) {
    const secureResult = await authClient.verifyPhoneChangeSecure({
    ...
    if (secureResult.completed === false) {
      ...
      formState.value.message = 'Verification code sent to your current phone'
      formState.value.messageType = 'info'
      formState.value.loading = false
    ...
    result = await authClient.verifyPhoneChange({
      phone: form.newPhone,
      token: form.verificationCode,
      session_code: sessionCode.value
    })
